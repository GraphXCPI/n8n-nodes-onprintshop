...
exports.ONPRINTSHOP_FIELD_SELECTION_MODE_SUFFIX = exports.ONPRINTSHOP_POSTMAN_DOC_URL = exports.ONPRINTSHOP_OPERATOR_DOC_URL = void 0;
exports.isOnPrintShopFieldSelectionSentinel = isOnPrintShopFieldSelectionSentinel;
exports.cleanOnPrintShopFieldValues = cleanOnPrintShopFieldValues;
exports.addOnPrintShopHelp = addOnPrintShopHelp;
exports.ONPRINTSHOP_OPERATOR_DOC_URL = 'https://github.com/GraphXCPI/n8n-nodes-onprintshop/blob/main/docs/NODE_OPERATOR_GUIDE.md';
exports.ONPRINTSHOP_POSTMAN_DOC_URL = 'https://documenter.getpostman.com/view/33263100/2sBXijHWys#intro';
exports.ONPRINTSHOP_FIELD_SELECTION_MODE_SUFFIX = 'Mode';
const RESOURCE_HELP = {
...
function isOnPrintShopFieldSelectionSentinel(value) {
...
function cleanOnPrintShopFieldValues(values = []) {
...
function getOperationHelp(resource, option) {
    if (!option.value)
        return undefined;
    ...
    if (resourceHelp && optionText)
        return `${optionText}. ${resourceHelp}`;
    return resourceHelp || optionText;
}
function buildOperationNotice(resource, option) {
    if (!option.value)
        return undefined;
...
function operationNoticesFor(property) {
...
function fieldSelectionModeName(propertyName) {
    return `${propertyName}${exports.ONPRINTSHOP_FIELD_SELECTION_MODE_SUFFIX}`;
}
function supportsFieldSelectionMode(property) {
    return property.type === 'multiOptions'
        && typeof property.name === 'string'
        && property.name.toLowerCase().includes('fields')
        && Array.isArray(property.options)
        && property.options.some((option) => typeof option === 'object' && option !== null && typeof option.value === 'string');
}
function buildFieldSelectionMode(property) {
    if (!supportsFieldSelectionMode(property))
        return undefined;
    return {
...
function cleanFieldSelectionProperty(property) {
    var _a;
    if (!supportsFieldSelectionMode(property) || !Array.isArray(property.options))
        return property;
    const next = {
        ...property,
        options: property.options.filter((option) => {
            if (typeof option !== 'object' || option === null)
                return true;
            return !isOnPrintShopFieldSelectionSentinel(option.value);
        }),
        displayOptions: {
            ...property.displayOptions,
            show: {
                ...(_a = property.displayOptions) === null || _a === void 0 ? void 0 : _a.show,
...
function applyHelpToProperty(property) {
    const help = PARAMETER_HELP[property.name];
    const next = {
        ...property,
    };
    if (help === null || help === void 0 ? void 0 : help.description)
        next.description = help.description;
    if (help === null || help === void 0 ? void 0 : help.hint)
        next.hint = help.hint;
    if (help === null || help === void 0 ? void 0 : help.placeholder)
        next.placeholder = help.placeholder;
    if (Array.isArray(next.options)) {
        next.options = next.options.map((option) => {
            if (typeof option !== 'object' || option === null)
                return option;
            if (!('displayName' in option) || !('type' in option) || !('name' in option))
                return option;
            return applyHelpToProperty(option);
        });
    }
    return cleanFieldSelectionProperty(next);
}
function addOnPrintShopHelp(description) {
    const properties = [];
    for (const property of description.properties) {
        const mode = buildFieldSelectionMode(property);
        if (mode)
            properties.push(applyHelpToProperty(mode));
