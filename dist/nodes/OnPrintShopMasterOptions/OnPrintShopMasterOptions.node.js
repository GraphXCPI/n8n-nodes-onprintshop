...
const numberField = (displayName, name, resource, operations, defaultValue = 0, required = false, description) => ({
    displayName,
    name,
    type: 'number',
...
const stringField = (displayName, name, resource, operations, required = false, description) => ({
    displayName,
    name,
    type: 'string',
    default: '',
...
const paginationFields = (resource, operations) => [
...
const operationProperty = (resource, options, defaultValue) => ({
...
const fixedCollection = (displayName, name, resource, operation, groupName, groupDisplayName, values, required = true, multipleValues = true) => ({
    displayName,
    name,
...
const nestedFixedCollection = (displayName, name, groupName, groupDisplayName, values, required = false) => ({
    displayName,
    name,
    type: 'fixedCollection',
...
const attributeValues = [
    ...
    { displayName: 'Image Filename', name: 'attributes_image', type: 'string', default: '', description: 'Existing uploaded filename under images/product_icon' },
    { displayName: 'Image URL', name: 'attributes_image_url', type: 'string', default: '', description: 'Direct image URL accessible to OnPrintShop; omit Image Filename when using this field' },
...
function normalizeNestedRows(row) {
    const normalized = (0, OnPrintShopGraphqlRequest_1.compactObject)(row);
...
function parseObjectArray(value, node, itemIndex, label = 'Batch input') {
    let parsed = value;
    if (typeof value === 'string') {
        try {
            ...
            throw new n8n_workflow_1.NodeOperationError(node, `${label} must be valid JSON: ${error.message}`, { itemIndex });
        }
    }
    if (!Array.isArray(parsed)) {
        throw new n8n_workflow_1.NodeOperationError(node, `${label} must be an array of objects`, { itemIndex });
    }
    return parsed.map((entry, index) => {
        if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
            throw new n8n_workflow_1.NodeOperationError(node, `${label} item ${index + 1} must be an object`, { itemIndex });
        }
        return normalizeNestedRows(entry);
    });
}
function requiredResult(data, field, node, itemIndex) {
    if (data[field] === undefined) {
        throw new n8n_workflow_1.NodeOperationError(node, `OnPrintShop response did not include ${field}`, { itemIndex });
    }
    return data[field];
}
class OnPrintShopMasterOptions {
    constructor() {
        this.description = {
        ...
        for (let itemIndex = 0; itemIndex < inputItems.length; itemIndex++) {
            try {
                ...
                if (resource === 'masterOption' && operation === 'getMany') {
                    const variables = { limit: this.getNodeParameter('masterOptionLimit', itemIndex), offset: this.getNodeParameter('masterOptionOffset', itemIndex) };
                    ...
                    const inputMode = this.getNodeParameter('masterOptionInputMode', itemIndex, 'form');
                    const inputs = inputMode === 'json'
                        ...
                        : (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('masterOptions', itemIndex), 'item').map(normalizeNestedRows);
                    ...
                    const inputMode = this.getNodeParameter('attributeInputMode', itemIndex, 'form');
                    const inputs = inputMode === 'json'
                        ? parseObjectArray(this.getNodeParameter('attributesJson', itemIndex), this.getNode(), itemIndex, 'Attributes JSON')
                        : (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('attributes', itemIndex), 'item').map(normalizeNestedRows);
                    ...
                    const variables = { limit: this.getNodeParameter('attributePriceLimit', itemIndex), offset: this.getNodeParameter('attributePriceOffset', itemIndex) };
                    const id = this.getNodeParameter('attributePriceId', itemIndex);
                    if (id)
                        variables.attr_id = id;
                    ...
                    const inputMode = this.getNodeParameter('attributePriceInputMode', itemIndex, 'form');
                    const inputs = inputMode === 'json'
                        ? parseObjectArray(this.getNodeParameter('attributePricesJson', itemIndex), this.getNode(), itemIndex, 'Attribute Prices JSON')
                        : (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('attributePrices', itemIndex), 'item').map(normalizeNestedRows);
                    ...
                    const variables = { limit: this.getNodeParameter('rangeLimit', itemIndex), offset: this.getNodeParameter('rangeOffset', itemIndex) };
                    const rangeId = this.getNodeParameter('rangeId', itemIndex);
                    const optionId = this.getNodeParameter('rangeOptionId', itemIndex);
                    if (rangeId)
                    ...
                    const input = { option_id: this.getNodeParameter('setRangeOptionId', itemIndex), ranges: (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('ranges', itemIndex), 'range').map(normalizeNestedRows) };
                    if (this.getNodeParameter('deleteAllRanges', itemIndex))
                        input.delete = 1;
                    ...
                    const variables = { limit: this.getNodeParameter('tagLimit', itemIndex), offset: this.getNodeParameter('tagOffset', itemIndex) };
                    const id = this.getNodeParameter('tagId', itemIndex);
                    if (id)
                        variables.master_option_tag_id = id;
                    ...
                    const input = { master_option_tag_id: this.getNodeParameter('setTagId', itemIndex), master_option_tag_name: this.getNodeParameter('tagName', itemIndex), delete: this.getNodeParameter('deleteTag', itemIndex) ? 1 : 0 };
                    ...
                    const variables = { limit: this.getNodeParameter('optionGroupLimit', itemIndex), offset: this.getNodeParameter('optionGroupOffset', itemIndex) };
                    const id = this.getNodeParameter('optionGroupId', itemIndex);
                    const useFor = this.getNodeParameter('optionGroupUseFor', itemIndex);
                    if (id)
                        variables.prod_add_opt_group_id = id;
                    if (useFor)
                    ...
                    const variables = { limit: this.getNodeParameter('formulaLimit', itemIndex), offset: this.getNodeParameter('formulaOffset', itemIndex) };
                    const id = this.getNodeParameter('formulaId', itemIndex);
                    if (id)
                        variables.formula_id = id;
                    ...
                    const input = { formula_id: this.getNodeParameter('setFormulaId', itemIndex), formula_label: this.getNodeParameter('formulaLabel', itemIndex), formula_syntax: this.getNodeParameter('formulaSyntax', itemIndex), delete: this.getNodeParameter('deleteFormula', itemIndex) ? 1 : 0 };
                    ...
                    const variables = { limit: this.getNodeParameter('ruleLimit', itemIndex), offset: this.getNodeParameter('ruleOffset', itemIndex) };
                    const id = this.getNodeParameter('ruleId', itemIndex);
                    if (id)
                        variables.rule_id = id;
                    ...
                    const rules = (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('rules', itemIndex), 'item').map(normalizeNestedRows);
                    if (rules.length !== 1)
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Exactly one product option rule is required', { itemIndex });
                    const input = rules[0];
                    ...
                    if (operation === 'getConfigs') {
                        const variables = { limit: this.getNodeParameter('stockLimit', itemIndex), offset: this.getNodeParameter('stockOffset', itemIndex) };
                        const configId = this.getNodeParameter('stockConfigId', itemIndex);
                        ...
                        const configId = this.getNodeParameter('stockHistoryConfigId', itemIndex);
                        if (configId <= 0)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Config ID must be greater than zero', { itemIndex });
                        const variables = { config_id: configId, limit: this.getNodeParameter('stockLimit', itemIndex), offset: this.getNodeParameter('stockOffset', itemIndex) };
                        ...
                        if (optionIds.length < 2)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Combination Matrix requires at least two option IDs', { itemIndex });
                        ...
                        const input = { stock_type: this.getNodeParameter('stockType', itemIndex), option_id: this.getNodeParameter('stockOptionId', itemIndex), default_stock: this.getNodeParameter('stockDefault', itemIndex), custom_multiplier: this.getNodeParameter('stockCustomMultiplier', itemIndex) ? 1 : 0, apply_to_all: this.getNodeParameter('stockApplyToAll', itemIndex), multipliers: (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('stockMultipliers', itemIndex), 'multiplier').map(normalizeNestedRows), attributes: (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('stockAttributes', itemIndex), 'attribute').map(normalizeNestedRows), combinations: (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('stockCombinations', itemIndex), 'combination').map(normalizeNestedRows) };
                        data = await request(`mutation addMasterOptionStockConfig($input: AddMasterOptionStockConfigInput!) { addMasterOptionStockConfig(input: $input) { index result message id } }`, { input: (0, OnPrintShopGraphqlRequest_1.compactObject)(input) }, itemIndex);
                        result = requiredResult(data, 'addMasterOptionStockConfig', this.getNode(), itemIndex);
                    }
                    else if (operation === 'deleteConfig') {
                        const variables = {};
                        const configId = this.getNodeParameter('stockConfigId', itemIndex);
                        const optionIds = this.getNodeParameter('stockDeleteOptionIds', itemIndex);
                        if (Boolean(configId) === Boolean(optionIds))
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Provide exactly one of Config ID or Option IDs', { itemIndex });
                        if (configId)
                            variables.config_id = configId;
                        if (optionIds)
                        ...
                        const inputMode = this.getNodeParameter('stockChangeInputMode', itemIndex, 'form');
                        const inputs = inputMode === 'json'
                            ? parseObjectArray(this.getNodeParameter('stockChangesJson', itemIndex), this.getNode(), itemIndex, 'Stock Changes JSON')
                            : (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('stockChanges', itemIndex), 'item').map(normalizeNestedRows);
                        ...
                        const inputMode = this.getNodeParameter('stockSettingsInputMode', itemIndex, 'form');
                        const inputs = inputMode === 'json'
                            ? parseObjectArray(this.getNodeParameter('stockSettingsJson', itemIndex), this.getNode(), itemIndex, 'Stock Settings JSON')
                            : (0, OnPrintShopGraphqlRequest_1.rowsFromFixedCollection)(this.getNodeParameter('stockSettings', itemIndex), 'item').map(normalizeNestedRows);
                    ...
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Unsupported Master Options operation ${resource}.${operation}`, { itemIndex });
                }
                output.push(...(0, OnPrintShopGraphqlRequest_1.resultItems)(result, itemIndex));
            }
            catch (error) {
...
exports.OnPrintShopMasterOptions = OnPrintShopMasterOptions;
