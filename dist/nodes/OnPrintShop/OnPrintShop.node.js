...
class OnPrintShop {
    constructor() {
        ...
        (0, OnPrintShopUploadInputs_1.addUploadInputs)(this.description);
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (resource === 'productStocks' && operation === 'getAll') {
                    ...
                    const fetchAllPages = this.getNodeParameter('fetchAllPages', i, false) || false;
                    const stockType = this.getNodeParameter('stockType', i, 'product');
                    const renamedStockFields = {
                    ...
                    if (fetchAllPages) {
                        while (hasMorePages && pageCount < maxPages) {
                            const requestStartTime = Date.now();
                            const variables = { type: stockType, limit: pageSize, offset };
                            const qp = queryParameters || {};
                            if (qp.product_id)
                        ...
                        const variables = { type: stockType };
                        ...
                        if (responseData && responseData.data && responseData.data.productStocks) {
                            ...
                            for (const stock of stocks)
                                returnData.push({ ...stock, _totalStocks: total, _currentCount: currentCount });
                        }
                        else if (responseData && responseData.errors) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
                        }
                ...
                if (resource === 'mutation') {
                    if (operation === 'updateOrderStatus') {
                        ...
                        const input = JSON.parse(this.getNodeParameter('updateOrderStatusInput', i));
                    ...
                    if (operation === 'setProduct') {
                        const inputs = toBatchInputs(parseJsonParameter((0, OnPrintShopUploadInputs_1.readUploadInput)(this, 'setProduct', i), 'setProduct_input', this.getNode(), i), 'setProduct_input', this.getNode(), i);
                    ...
                    if (operation === 'setProductImage') {
                        ...
                        const input = JSON.parse((0, OnPrintShopUploadInputs_1.readUploadInput)(this, 'setProductImage', i));
                    ...
                    if (operation === 'setOrderProductImageFromUrl') {
                        const order_product_id = this.getNodeParameter('urlUploadOrderProductId', i);
                        if (!Number.isInteger(order_product_id) || order_product_id <= 0)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Provide a positive Order Product ID', { itemIndex: i });
                        const input = (0, OnPrintShopUploadInputs_1.readOrderUrlInput)(this, i);
                        const options = this.getNodeParameter('urlUploadOptions', i, {});
                        ...
                        if ((_c = response.errors) === null || _c === void 0 ? void 0 : _c.length)
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'OnPrintShop rejected the URL upload. Check the order product, file accessibility, and input fields.', { itemIndex: i });
                        if ((_d = response.data) === null || _d === void 0 ? void 0 : _d.setOrderProductImageFromUrl)
                            returnData.push(response.data.setOrderProductImageFromUrl);
                    }
                    ...
                    if (operation === 'setMasterOptionAttributes') {
                        const inputs = toBatchInputs(parseJsonParameter((0, OnPrintShopUploadInputs_1.readUploadInput)(this, 'setMasterOptionAttributes', i), 'setMasterOptionAttributes_input', this.getNode(), i), 'setMasterOptionAttributes_input', this.getNode(), i);
                    ...
                    if (operation === 'setProductSize') {
                        const inputs = toBatchInputs(parseJsonParameter((0, OnPrintShopUploadInputs_1.readUploadInput)(this, 'setProductSize', i), 'setProductSize_input', this.getNode(), i), 'setProductSize_input', this.getNode(), i);
                    ...
                    if (operation === 'setProductCategory') {
                        const inputs = toBatchInputs(parseJsonParameter((0, OnPrintShopUploadInputs_1.readUploadInput)(this, 'setProductCategory', i), 'setProductCategory_input', this.getNode(), i), 'setProductCategory_input', this.getNode(), i);
                    ...
                    if (operation === 'setAdditionalOptionAttributes') {
                        const inputs = toBatchInputs(parseJsonParameter((0, OnPrintShopUploadInputs_1.readUploadInput)(this, 'setAdditionalOptionAttributes', i), 'setAdditionalOptionAttributes_input', this.getNode(), i), 'setAdditionalOptionAttributes_input', this.getNode(), i);
        ...
        shipToMultipleAddress {
			...
			ship_to_multiple_address_extra_field
                    ...
                    if (responseData && responseData.data && responseData.data.productStocks) {
                        ...
                        if (Array.isArray(stocks) && stocks.length > 0) {
                            ...
                            returnData.push({
                                ...
                                productId: productId !== null && productId !== void 0 ? productId : null,
                            });
                        }
                    }
                    else if (responseData && responseData.errors) {
                ...
                if (resource === 'product' && operation === 'updateStock') {
                    const rawParameters = (this.getNode().parameters || {});
                    ...
                    const inputMode = hasInputMode
                        ? this.getNodeParameter('stockDetailsInputMode', i, 'form')
                        : 'legacy';
                    let stockDetails = [];
                    if (inputMode === 'json') {
                        const parsed = parseJsonParameter(this.getNodeParameter('stockDetailsJson', i), 'stockDetailsJson', this.getNode(), i);
                        if (!Array.isArray(parsed)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Stock Details must be a JSON object array', { itemIndex: i });
                        }
                        stockDetails = parsed;
                    }
                    else if (inputMode === 'form') {
                        const collection = this.getNodeParameter('stockDetails', i, {});
                        stockDetails = Array.isArray(collection.values) ? collection.values : [];
                    }
                    else {
                        const identifierType = this.getNodeParameter('stockIdentifierType', i, 'stock_id');
                        const legacyAction = String(this.getNodeParameter('stockAction', i, 'Set')).toLowerCase();
                        const legacyDetail = {
                            stock_change: this.getNodeParameter('stock_quantity', i),
                            action: legacyAction === 'set' ? 'reset' : legacyAction,
                            comment: this.getNodeParameter('comment', i, ''),
                        };
                        if (identifierType === 'stock_id') {
                            const stockId = parseInt(this.getNodeParameter('stockId', i), 10);
                            ...
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Product SKU is required for a legacy SKU stock update', { itemIndex: i });
                        }
                        stockDetails = [legacyDetail];
                    }
                    if (stockDetails.length === 0) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'At least one Stock Detail is required', { itemIndex: i });
                    }
                    ...
                    stockDetails = stockDetails.map((detail, detailIndex) => {
                        ...
                        if ((detail.stock_id !== undefined && !Number.isFinite(stockId)) || (detail.stock_id === undefined && !productSku) || !Number.isFinite(stockChange) || !allowedActions.has(action)) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Stock Detail ${detailIndex + 1} requires numeric stock_id, numeric stock_change, and action add, remove, or reset`, { itemIndex: i });
                        }
                        const normalized = { stock_change: stockChange, action };
                        if (detail.stock_id !== undefined)
                            normalized.stock_id = stockId;
                        if (detail.comment !== undefined && detail.comment !== '')
                            normalized.comment = String(detail.comment);
                        if (detail.location !== undefined && detail.location !== '')
                            normalized.location = String(detail.location);
                        return normalized;
                    });
                    const variables = { input: { stock_details: stockDetails } };
                    const updateStockType = this.getNodeParameter('updateStockType', i, '');
                    if (updateStockType)
                        variables.type = updateStockType;
                    if (productSku)
			...
			`;
                    ...
                    while (hasMoreData) {
                        ...
                        if (fetchAllPages) {
                            ...
                            const totalFaq = typeof ((_f = (_e = responseData === null || responseData === void 0 ? void 0 : responseData.data) === null || _e === void 0 ? void 0 : _e.faq) === null || _f === void 0 ? void 0 : _f.totalFaq) === 'number' ? responseData.data.faq.totalFaq : 0;
