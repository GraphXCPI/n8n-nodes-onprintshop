...
import {
	addOnPrintShopHelp,
	cleanOnPrintShopFieldValues,
	ONPRINTSHOP_FIELD_SELECTION_MODE_SUFFIX,
...
function replaceGraphqlTokens(query: string, aliases: Record<string, string>): string {
	let fieldDepth = 0;
...
function toNodeApiErrorResponse(error: unknown): JsonObject {
	if (isPlainObject(error)) return error as JsonObject;
	return { message: getErrorMessage(error) };
}

function prepareVariablesForOpsSchema(query: string, variables: IDataObject): IDataObject {
	...
	return normalizeOnPrintShopInputs(query, prepared);
}

function parseJsonParameter(rawValue: string, parameterName: string, node: INode, itemIndex: number): unknown {
	try {
...
function propertyMatchesExecutionContext(property: INodeProperties, resource: string, operation: string): boolean {
...
function getFieldOptionsForExecution(
	...
	return cleanOnPrintShopFieldValues(
		property.options
...
function hasFieldSelectionModeForExecution(
	description: INodeTypeDescription,
	parameterName: string,
	resource: string,
	operation: string,
): boolean {
	const modeParameterName = `${parameterName}${ONPRINTSHOP_FIELD_SELECTION_MODE_SUFFIX}`;
...
function resolveFieldSelection(
	...
	mode: OnPrintShopFieldSelectionMode,
	resource: string,
	operation: string,
): string[] {
	if (mode === 'all') return getFieldOptionsForExecution(description, parameterName, resource, operation);
	if (mode === 'none') return [];
	return cleanOnPrintShopFieldValues(selectedFields);
}

function toBatchInputs(value: unknown, parameterName: string, node: INode, itemIndex: number): IDataObject[] {
	if (!Array.isArray(value)) {
		throw new NodeOperationError(
			node,
			`${parameterName} must be a JSON array of input objects, e.g. [{ "...": "..." }]. Bare object or wrapper inputs are not supported for batchable OPS mutations.`,
			{ itemIndex },
		);
	}
...
function addLegacyBatchIdAlias(result: IDataObject): IDataObject {
...
function addResponseAliases(value: unknown): unknown {
	if (Array.isArray(value)) {
	...
	for (const [key, nestedValue] of Object.entries(value)) {
		normalized[key] = addResponseAliases(nestedValue) as IDataObject;
...
export class OnPrintShop implements INodeType {
	description: INodeTypeDescription = {
		...
		documentationUrl: ONPRINTSHOP_OPERATOR_DOC_URL,
		codex: {
			categories: ['Sales'],
			subcategories: {
				Sales: ['Ecommerce'],
			...
			resources: {
				primaryDocumentation: [
					{
						url: ONPRINTSHOP_OPERATOR_DOC_URL,
					},
				],
	...
	constructor() {
		...
		for (const [resource, operation, sourceResource, sourceOperation] of [
			...
			for (const property of [...this.description.properties]) {
				if (property.name === 'operation' || !getShowValues(property, 'resource')?.includes(sourceResource)
					|| !getShowValues(property, 'operation')?.includes(sourceOperation)) continue;
				const copy = JSON.parse(JSON.stringify(property)) as INodeProperties;
				copy.displayOptions = { ...copy.displayOptions, show: { ...copy.displayOptions?.show, resource: [resource], operation: [operation] } };
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (operation === 'getMany' && ['customer', 'customerAddress', 'order', 'orderDetails', 'orderShipment'].includes(resource)) operation = 'getAll';
				if (resource === 'faq' && operation === 'getMany') { resource = 'product'; operation = 'getManyFAQs'; }
				const getFieldSelection = (parameterName: string, fallback: string[] = []): string[] => {
					const selectedFields = this.getNodeParameter(parameterName, i, fallback) as string[];
					const mode = hasFieldSelectionModeForExecution(nodeDescription, parameterName, resource, operation)
						? this.getNodeParameter(
							`${parameterName}${ONPRINTSHOP_FIELD_SELECTION_MODE_SUFFIX}`,
							i,
							'custom',
						) as OnPrintShopFieldSelectionMode
						: 'custom';
			...
			if (resource === 'graphql' && operation === 'execute') {
				...
				const variables = JSON.parse(this.getNodeParameter('graphqlVariables', i) as string || '{}');
				...
				if (resource === 'masterOptionStock') {
					if (operation === 'getConfigs') {
						const optionFilter = parseJsonParameter(this.getNodeParameter('masterOptionStock_optionFilter', i) as string, 'masterOptionStock_optionFilter', this.getNode(), i);
						if (!Array.isArray(optionFilter)) {
							throw new NodeOperationError(this.getNode(), 'Option Filter must be a JSON array, e.g. [52,59]', { itemIndex: i });
						}
						const variables: IDataObject = {
							limit: this.getNodeParameter('masterOptionStock_limit', i) as number,
							offset: this.getNodeParameter('masterOptionStock_offset', i) as number,
						};
						const configId = this.getNodeParameter('masterOptionStock_configId', i) as number;
						if (configId) variables.config_id = configId;
						if (optionFilter.length) variables.optionFilter = optionFilter as IDataObject[];
						...
						if (responseData && responseData.data && responseData.data.getMasterOptionStockConfigs) returnData.push(responseData.data.getMasterOptionStockConfigs);
					...
					if (operation === 'getHistory') {
						const configId = this.getNodeParameter('masterOptionStock_configId', i) as number;
						if (!configId) throw new NodeOperationError(this.getNode(), 'Config ID is required for master option stock history', { itemIndex: i });
						const variables: IDataObject = {
							config_id: configId,
							limit: this.getNodeParameter('masterOptionStock_limit', i) as number,
							offset: this.getNodeParameter('masterOptionStock_offset', i) as number,
						...
						const responseData = await requestGraphql({ query, variables });
						if (responseData && responseData.data && responseData.data.getMasterOptionStockHistory) returnData.push(responseData.data.getMasterOptionStockHistory);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
					if (operation === 'getCombinationMatrix') {
						const optionIds = parseJsonParameter(this.getNodeParameter('masterOptionStock_optionIdsArray', i) as string, 'masterOptionStock_optionIdsArray', this.getNode(), i);
						if (!Array.isArray(optionIds) || optionIds.length === 0) {
							throw new NodeOperationError(this.getNode(), 'Option IDs must be a non-empty JSON array, e.g. [50,52]', { itemIndex: i });
						}
						...
						if (responseData && responseData.data && responseData.data.getMasterOptionCombinationMatrix) returnData.push(responseData.data.getMasterOptionCombinationMatrix);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
					if (operation === 'addConfig') {
						const input = parseJsonParameter(this.getNodeParameter('masterOptionStock_input', i) as string, 'masterOptionStock_input', this.getNode(), i) as IDataObject;
						...
						if (responseData && responseData.data && responseData.data.addMasterOptionStockConfig) returnData.push(responseData.data.addMasterOptionStockConfig);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
					if (operation === 'deleteConfig') {
						const variables: IDataObject = {};
						const configId = this.getNodeParameter('masterOptionStock_configId', i) as number;
						const optionIds = this.getNodeParameter('masterOptionStock_optionIds', i) as string;
						if (configId) variables.config_id = configId;
						if (optionIds) variables.option_ids = optionIds;
						if (variables.config_id === undefined && variables.option_ids === undefined) {
							throw new NodeOperationError(this.getNode(), 'Either Config ID or Option IDs is required to delete a master option stock config', { itemIndex: i });
						}
						...
						if (responseData && responseData.data && responseData.data.deleteMasterOptionStockConfig) returnData.push(responseData.data.deleteMasterOptionStockConfig);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
					if (operation === 'updateStock') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('masterOptionStock_inputs', i) as string, 'masterOptionStock_inputs', this.getNode(), i), 'masterOptionStock_inputs', this.getNode(), i);
						...
						if (responseData && responseData.data && responseData.data.updateMasterOptionStock) returnData.push(responseData.data.updateMasterOptionStock);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
					if (operation === 'setSettings') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('masterOptionStock_settingsInputs', i) as string, 'masterOptionStock_settingsInputs', this.getNode(), i), 'masterOptionStock_settingsInputs', this.getNode(), i);
						...
						const responseData = await requestGraphql({ query: mutation, variables: { inputs } });
						if (responseData && responseData.data && responseData.data.setMasterOptionStockSettings) returnData.push(responseData.data.setMasterOptionStockSettings);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}

				if (resource === 'product' && operation === 'getSkuMatrix') {
					const products_id = this.getNodeParameter('productSkuMatrix_products_id', i) as number;
					const prod_add_opt_ids = this.getNodeParameter('productSkuMatrix_prod_add_opt_ids', i) as string;
					if (!products_id) throw new NodeOperationError(this.getNode(), 'Product ID is required for product SKU matrix', { itemIndex: i });
					if (!prod_add_opt_ids) throw new NodeOperationError(this.getNode(), 'Product Additional Option IDs are required for product SKU matrix', { itemIndex: i });
					...
					if (responseData && responseData.data && responseData.data.getProductSkuMatrix) returnData.push(responseData.data.getProductSkuMatrix);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}

				if ((resource === 'product' || resource === 'mutation') && operation === 'setProductSku') {
					const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setProductSku_input', i) as string, 'setProductSku_input', this.getNode(), i), 'setProductSku_input', this.getNode(), i);
					const mutation = `mutation setProductSku($inputs: [ProductSkuInput!]!) { setProductSku(inputs: $inputs) { index result message id } }`;
					const responseData = await requestGraphql({ query: mutation, variables: { inputs } });
					if (responseData && responseData.data && responseData.data.setProductSku) returnData.push(responseData.data.setProductSku);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				...
				if (resource === 'storeMarkup' && operation === 'getAll') {
					...
					const fetchAllPages = this.getNodeParameter('storeMarkupFetchAllPages', i, false) as boolean;
					const pageSize = 250;
					let offset = 0;
					let hasMore = true;
					do {
				...
				if (resource === 'product' && operation === 'getManyOptionGroups') {
					const queryParameters = this.getNodeParameter('queryParametersManyOptionGroups', i, {}) as IDataObject;
					const variables: IDataObject = {};
					if (queryParameters.prod_add_opt_group_id) variables.prod_add_opt_group_id = Number(queryParameters.prod_add_opt_group_id);
					if (queryParameters.use_for) variables.use_for = String(queryParameters.use_for);
					...
					if (responseData && responseData.data && responseData.data.getOptionGroup) {
						for (const group of responseData.data.getOptionGroup.optionGroup || []) returnData.push({ ...group, _totalOptionGroup: responseData.data.getOptionGroup.totalOptionGroup });
					} else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}

				if (resource === 'product' && operation === 'getManyMasterOptionRanges') {
					const queryParameters = this.getNodeParameter('queryParametersManyMasterOptionRanges', i, {}) as IDataObject;
					const variables: IDataObject = {};
					if (queryParameters.range_id) variables.range_id = Number(queryParameters.range_id);
					if (queryParameters.option_id) variables.option_id = Number(queryParameters.option_id);
					variables.limit = queryParameters.limit || 50;
					variables.offset = queryParameters.offset || 0;
					...
					if (responseData && responseData.data && responseData.data.getMasterOptionRange) {
						for (const range of responseData.data.getMasterOptionRange.masterOptionRange || []) returnData.push({ ...range, _totalMasterOptionRange: responseData.data.getMasterOptionRange.totalMasterOptionRange });
					} else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}

				if (resource === 'product' && operation === 'getManyMasterOptionTags') {
					const queryParameters = this.getNodeParameter('queryParametersManyMasterOptionTags', i, {}) as IDataObject;
					const variables: IDataObject = {};
					if (queryParameters.master_option_tag_id) variables.master_option_tag_id = Number(queryParameters.master_option_tag_id);
					variables.limit = queryParameters.limit || 50;
					variables.offset = queryParameters.offset || 0;
					...
					if (responseData && responseData.data && responseData.data.getMasterOptionTag) {
						for (const tag of responseData.data.getMasterOptionTag.masterOptionTag || []) returnData.push({ ...tag, _totalMasterOptionTag: responseData.data.getMasterOptionTag.totalMasterOptionTag });
					} else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				...
				if (resource === 'orderProducts' && (operation === 'get' || operation === 'getMany')) {
					...
					const variables: IDataObject = {};
					if (orderProductId) variables.orders_products_id = Number(orderProductId);
					variables.limit = 50;
					variables.offset = 0;
					...
					if (responseData && responseData.data && responseData.data.orders) {
						for (const order of responseData.data.orders.orders || []) returnData.push({ ...order, _totalOrders: responseData.data.orders.totalOrders });
					} else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}

				if (resource === 'orderProducts' && operation === 'setDesign') {
					const order_product_id = Number(this.getNodeParameter('orderProductIdSetDesign', i) as string);
					const ziflow_link = this.getNodeParameter('ziflowLink', i, '') as string;
					const ziflow_preflight_link = this.getNodeParameter('ziflowPreflightLink', i, '') as string;
				...
				if (resource === 'orderProducts' && operation === 'updateStatus') {
					const orders_products_id = Number(this.getNodeParameter('orderProductIdUpdate', i) as string);
					const order_product_status = this.getNodeParameter('orderProductStatusUpdate', i) as string;
					const additionalFields = this.getNodeParameter('additionalFieldsProductUpdate', i, {}) as IDataObject;
					const input: IDataObject = { order_product_status, comment: additionalFields.comment || '', notify: additionalFields.notify || 0 };
			...
			if (resource === 'customerAddress' && operation === 'getAll') {
				...
				if (resource === 'orderShipment' && operation === 'getAll') {
					...
					const allShipments: IDataObject[] = [];
					let offset = 0;
					let hasMorePages = true;
					const pageSize = Math.min((queryParameters.pageSize as number) || 250, 250);
					let adaptiveDelay = Math.max((queryParameters.pageDelay as number) || 50, 25);
				...
				if (resource === 'shipToMultipleAddress' && operation === 'getAll') {
					...
					if (!Number.isInteger(Number(queryParameters.orders_id)) || Number(queryParameters.orders_id) <= 0) {
						throw new NodeOperationError(this.getNode(), 'Set a positive Orders ID in Query Parameters to retrieve ship-to-multiple addresses', { itemIndex: i });
					}
					...
					if (fetchAllPages) {
						while (hasMorePages && pageCount < maxPages) {
							...
							if (responseData && responseData.data && responseData.data.shipToMultipleAddress) {
								...
								offset += pageSize; pageCount++; hasMorePages = addresses.length === pageSize;
								const responseTime = Date.now() - requestStartTime;
								if (responseTime < 100) adaptiveDelay = Math.max(25, adaptiveDelay * 0.8); else if (responseTime > 500) adaptiveDelay = Math.min(1000, adaptiveDelay * 1.25);
								if (hasMorePages) await sleep(Math.round(adaptiveDelay));
							} else if (responseData && responseData.errors) {
				...
				if (resource === 'status') {
					...
					const statusAliases: Record<string, string> = { id: 'id: process_status_id', title: 'title: process_status_title' };
					...
					if (operation === 'orderStatus' || operation === 'orderProductStatus') {
						...
						if (responseData && responseData.data && responseData.data.orderStatus) returnData.push(...responseData.data.orderStatus.orderStatus);
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'countries' && operation === 'getAll') {
					const variables: IDataObject = {};
					const countriesId = this.getNodeParameter('countries_countriesId', i) as number;
					...
					if (countriesId) variables.countries_id = countriesId;
					...
					if (responseData && responseData.data && responseData.data.get_countries) {
						...
						for (const c of countries) { returnData.push({ ...c, _totalCountries: responseData.data.get_countries.totalCountries }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'faqCategory' && operation === 'getAll') {
					const variables: IDataObject = {};
					const faqcatId = this.getNodeParameter('faqCategory_faqcatId', i) as number;
					...
					if (faqcatId) variables.faqcat_id = faqcatId;
					if (status) variables.status = status;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.get_faq_category) {
						...
						for (const c of categories) { returnData.push({ ...c, _totalFaqCategory: responseData.data.get_faq_category.totalFaqCategory }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'markupMaster' && operation === 'getAll') {
					const variables: IDataObject = {};
					const corporateMarkupId = this.getNodeParameter('markupMaster_corporateMarkupId', i) as number;
					const limit = this.getNodeParameter('markupMaster_limit', i) as number;
					const offset = this.getNodeParameter('markupMaster_offset', i) as number;
					if (corporateMarkupId) variables.corporate_markup_id = corporateMarkupId;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.get_store_markup) {
						...
						for (const m of markups) { returnData.push({ ...m, _totalStoreMarkup: responseData.data.get_store_markup.totalStoreMarkup }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'masterOptionRanges' && operation === 'getAll') {
					const variables: IDataObject = {};
					const rangeId = this.getNodeParameter('masterOptionRanges_rangeId', i) as number;
					const optionId = this.getNodeParameter('masterOptionRanges_optionId', i) as number;
					const limit = this.getNodeParameter('masterOptionRanges_limit', i) as number;
					const offset = this.getNodeParameter('masterOptionRanges_offset', i) as number;
					...
					if (responseData && responseData.data && responseData.data.getMasterOptionRange) {
						...
						for (const r of ranges) { returnData.push({ ...r, _totalMasterOptionRange: responseData.data.getMasterOptionRange.totalMasterOptionRange }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'masterOptionTag' && operation === 'getAll') {
					const variables: IDataObject = {};
					const tagId = this.getNodeParameter('masterOptionTag_tagId', i) as number;
					...
					if (tagId) variables.master_option_tag_id = tagId;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.getMasterOptionTag) {
						...
						for (const t of tags) { returnData.push({ ...t, _totalMasterOptionTag: responseData.data.getMasterOptionTag.totalMasterOptionTag }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'optionFormulas' && operation === 'getAll') {
					const variables: IDataObject = {};
					const formulaId = this.getNodeParameter('optionFormulas_formulaId', i) as number;
					const limit = this.getNodeParameter('optionFormulas_limit', i) as number;
					const offset = this.getNodeParameter('optionFormulas_offset', i) as number;
					if (formulaId) variables.formula_id = formulaId;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.getCustomFormula) {
						...
						for (const f of formulas) { returnData.push({ ...f, _totalCustomFormula: responseData.data.getCustomFormula.totalCustomFormula }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'optionGroup' && operation === 'getAll') {
					const variables: IDataObject = {};
					const groupId = this.getNodeParameter('optionGroup_groupId', i) as number;
					const useFor = this.getNodeParameter('optionGroup_useFor', i) as string;
					...
					if (groupId) variables.prod_add_opt_group_id = groupId;
					...
					if (responseData && responseData.data && responseData.data.getOptionGroup) {
						...
						for (const g of groups) { returnData.push({ ...g, _totalOptionGroup: responseData.data.getOptionGroup.totalOptionGroup }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'paymentTerms' && operation === 'getAll') {
					const variables: IDataObject = {};
					const termId = this.getNodeParameter('paymentTerms_termId', i) as number;
					const limit = this.getNodeParameter('paymentTerms_limit', i) as number;
					const offset = this.getNodeParameter('paymentTerms_offset', i) as number;
					if (termId) variables.term_id = termId;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.get_payment_term_master) {
						...
						for (const t of terms) { returnData.push({ ...t, _totalPaymentTermMaster: responseData.data.get_payment_term_master.totalPaymentTermMaster }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'storeAddress' && operation === 'getAll') {
					const variables: IDataObject = {};
					const corporateId = this.getNodeParameter('storeAddress_corporateId', i) as number;
					const corporateAddressId = this.getNodeParameter('storeAddress_corporateAddressId', i) as number;
					...
					if (corporateId) variables.corporate_id = corporateId;
					if (corporateAddressId) variables.corporate_address_id = corporateAddressId;
					if (limit) variables.limit = limit;
					if (offset) variables.offset = offset;
					...
					if (responseData && responseData.data && responseData.data.storeaddress) {
						...
						for (const a of addresses) { returnData.push({ ...a, _totalStoreAddress: responseData.data.storeaddress.totalStoreAddress }); }
					} else if (responseData && responseData.errors) {
						throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
				}
				...
				if (resource === 'mutation') {
					...
					if (operation === 'setBatch') {
						...
						const responseData = await requestGraphql({ query: mutation, variables });
						if (responseData && responseData.data && responseData.data.setBatch) returnData.push(addLegacyBatchIdAlias(responseData.data.setBatch));
						else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
					}
					...
					if (operation === 'setProductPrice') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setProductPrice_input', i) as string, 'setProductPrice_input', this.getNode(), i), 'setProductPrice_input', this.getNode(), i);
					...
					if (operation === 'setShipment') {
						...
						const shipmentinfo = JSON.parse(this.getNodeParameter('setShipment_shipmentinfo', i) as string);
						const variables: IDataObject = { order_id, shipmentinfo };
						if (shipment_id) variables.shipment_id = shipment_id;
						if (tracking_number) variables.tracking_number = tracking_number;
					...
					if (operation === 'setMasterOptionRules' || operation === 'setProductOptionRules') {
						...
						const input = JSON.parse(this.getNodeParameter(inputParameterName, i) as string);
					...
					if (operation === 'setOptionFormulas') {
						const input = JSON.parse(this.getNodeParameter('setOptionFormulas_input', i) as string);
					...
					if (operation === 'setOptionGroup') {
						const input = JSON.parse(this.getNodeParameter('setOptionGroup_input', i) as string);
					...
					if (operation === 'setMasterOptionTags') {
						const input = JSON.parse(this.getNodeParameter('setMasterOptionTags_input', i) as string);
					...
					if (operation === 'setMasterOptionAttributePrice') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setMasterOptionAttributePrice_input', i) as string, 'setMasterOptionAttributePrice_input', this.getNode(), i), 'setMasterOptionAttributePrice_input', this.getNode(), i);
					...
					if (operation === 'setMasterOption') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setMasterOption_input', i) as string, 'setMasterOption_input', this.getNode(), i), 'setMasterOption_input', this.getNode(), i);
					...
					if (operation === 'assignOptions') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('assignOptions_input', i) as string, 'assignOptions_input', this.getNode(), i), 'assignOptions_input', this.getNode(), i);
					...
					if (operation === 'setProductPages') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setProductPages_input', i) as string, 'setProductPages_input', this.getNode(), i), 'setProductPages_input', this.getNode(), i);
					...
					if (operation === 'setStoreAddress') {
						const input = JSON.parse(this.getNodeParameter('setStoreAddress_input', i) as string);
					...
					if (operation === 'setDepartment') {
						const input = JSON.parse(this.getNodeParameter('setDepartment_input', i) as string);
					...
					if (operation === 'setStore') {
						const input = JSON.parse(this.getNodeParameter('setStore_input', i) as string);
					...
					if (operation === 'setFaqCategory') {
						const input = JSON.parse(this.getNodeParameter('setFaqCategory_input', i) as string);
					...
					if (operation === 'modifyOrderProduct') {
						...
						const input = JSON.parse(this.getNodeParameter('modifyOrderProduct_input', i) as string);
					...
					if (operation === 'setAdditionalOption') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setAdditionalOption_input', i) as string, 'setAdditionalOption_input', this.getNode(), i), 'setAdditionalOption_input', this.getNode(), i);
					...
					if (operation === 'setProductsAttributePrice') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setProductsAttributePrice_input', i) as string, 'setProductsAttributePrice_input', this.getNode(), i), 'setProductsAttributePrice_input', this.getNode(), i);
					...
					if (operation === 'setQuantityBasedAttributePrice') {
						const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter('setQuantityBasedAttributePrice_input', i) as string, 'setQuantityBasedAttributePrice_input', this.getNode(), i), 'setQuantityBasedAttributePrice_input', this.getNode(), i);
  ...
  setProductCategory(inputs: $inputs) {
    ...
    external_ref
				...
				if (resource === "product" && operation === "setAssignOptions") {
					const inputs = toBatchInputs(parseJsonParameter(this.getNodeParameter("setAssignOptions_input", i) as string, "setAssignOptions_input", this.getNode(), i), "setAssignOptions_input", this.getNode(), i);
    ...
    setMasterOptionAttributePrice (inputs: $inputs) {
        ...
        id
				...
				if (resource === "masterOption" && operation === "setMasterOptionRange") {
					const input = JSON.parse(this.getNodeParameter("setMasterOptionRange_input", i) as string);
    ...
    setMasterOptionRange (input: $input) {
        ...
        message
				...
				if (resource === "masterOption" && operation === "setMasterOptionTag") {
					const input = JSON.parse(this.getNodeParameter("setMasterOptionTag_input", i) as string);
    ...
    setOptionGroup (input: $input) {
        ...
        prod_add_opt_group_id
				...
				if (resource === "masterOption" && operation === "setCustomFormula") {
					const input = JSON.parse(this.getNodeParameter("setCustomFormula_input", i) as string);
    ...
    setProductOptionRules (input: $input) {
        ...
        rule_id
				...
				if (resource === "markup" && operation === "setStoreMarkup") {
					const input = JSON.parse(this.getNodeParameter("setStoreMarkup_input", i) as string);
    ...
    setStoreMarkup (input: $input) {
        ...
        id
				...
				if (resource === "markup" && operation === "getStoreMarkup") {
					const variables: IDataObject = {};
					const corporate_markup_id = this.getNodeParameter("getStoreMarkup_corporate_markup_id", i) as number;
					variables.corporate_markup_id = corporate_markup_id;
					const limit = this.getNodeParameter("getStoreMarkup_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getStoreMarkup_offset", i) as number;
  ...
  get_store_markup(corporate_markup_id: $corporate_markup_id, limit: $limit, offset: $offset) {
    ...
    totalStoreMarkup
					...
					if (responseData && responseData.data && responseData.data.get_store_markup) returnData.push(responseData.data.get_store_markup);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "masterOption" && operation === "getCustomFormula") {
					const variables: IDataObject = {};
					const formula_id = this.getNodeParameter("getCustomFormula_formula_id", i) as number;
					variables.formula_id = formula_id;
					const limit = this.getNodeParameter("getCustomFormula_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getCustomFormula_offset", i) as number;
					variables.offset = offset;
    ...
    getCustomFormula (formula_id: $formula_id, limit: $limit, offset: $offset) {
        ...
        totalCustomFormula
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.getCustomFormula) returnData.push(responseData.data.getCustomFormula);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "masterOption" && operation === "getOptionGroup") {
					const variables: IDataObject = {};
					const prod_add_opt_group_id = this.getNodeParameter("getOptionGroup_prod_add_opt_group_id", i) as number;
					variables.prod_add_opt_group_id = prod_add_opt_group_id;
					const use_for = this.getNodeParameter("getOptionGroup_use_for", i) as string;
					variables.use_for = use_for;
					const limit = this.getNodeParameter("getOptionGroup_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getOptionGroup_offset", i) as number;
					variables.offset = offset;
    ...
    getOptionGroup (prod_add_opt_group_id: $prod_add_opt_group_id,use_for: $use_for, limit: $limit, offset: $offset) {
        ...
        totalOptionGroup
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.getOptionGroup) returnData.push(responseData.data.getOptionGroup);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "masterOption" && operation === "getMasterOptionTag") {
					const variables: IDataObject = {};
					const master_option_tag_id = this.getNodeParameter("getMasterOptionTag_master_option_tag_id", i) as number;
					variables.master_option_tag_id = master_option_tag_id;
					const limit = this.getNodeParameter("getMasterOptionTag_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getMasterOptionTag_offset", i) as number;
					variables.offset = offset;
    ...
    getMasterOptionTag (master_option_tag_id: $master_option_tag_id, limit: $limit, offset: $offset) {
        ...
        totalMasterOptionTag
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.getMasterOptionTag) returnData.push(responseData.data.getMasterOptionTag);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "masterOption" && operation === "getMasterOptionRange") {
					const variables: IDataObject = {};
					const range_id = this.getNodeParameter("getMasterOptionRange_range_id", i) as number;
					variables.range_id = range_id;
					const option_id = this.getNodeParameter("getMasterOptionRange_option_id", i) as number;
					variables.option_id = option_id;
					const limit = this.getNodeParameter("getMasterOptionRange_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getMasterOptionRange_offset", i) as number;
					variables.offset = offset;
    ...
    getMasterOptionRange (range_id: $range_id, option_id: $option_id, limit: $limit, offset: $offset) {
        ...
        totalMasterOptionRange
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.getMasterOptionRange) returnData.push(responseData.data.getMasterOptionRange);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "product" && operation === "getFaqCategory") {
					const variables: IDataObject = {};
					const faqcat_id = this.getNodeParameter("getFaqCategory_faqcat_id", i) as number;
					variables.faqcat_id = faqcat_id;
					const status = this.getNodeParameter("getFaqCategory_status", i) as number;
					variables.status = status;
					const limit = this.getNodeParameter("getFaqCategory_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getFaqCategory_offset", i) as number;
					variables.offset = offset;
  ...
  get_faq_category(faqcat_id: $faqcat_id, status: $status, limit: $limit, offset: $offset) {
    ...
    totalFaqCategory
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.get_faq_category) returnData.push(responseData.data.get_faq_category);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
    ...
    setProductSize (inputs: $inputs) {
        ...
        id
				...
				if (resource === "product" && operation === "setFaq") {
					const input = JSON.parse(this.getNodeParameter("setFaq_input", i) as string);
  ...
  setFaqCategory(input: $input) {
    ...
    storeaddress(corporate_id: $corporate_id, corporate_address_id: $corporate_address_id, limit: $limit, offset: $offset) {
        ...
        totalStoreAddress
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.storeaddress) returnData.push(responseData.data.storeaddress);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
    ...
    setDepartment (input: $input) {
        ...
        id
				...
				if (resource === "customerAddress" && operation === "setCustomerAddressDetail") {
					const input = JSON.parse(this.getNodeParameter("setCustomerAddressDetail_input", i) as string);
    ...
    setCustomerAddressDetail (input: $input) {
        ...
        id
				...
				if (resource === "customer" && operation === "notifyUser") {
					...
					const input = JSON.parse(this.getNodeParameter("notifyUser_input", i) as string);
    ...
    notifyUser (cust_id: $cust_id, usertype: $usertype, input: $input) {
        ...
        id
				...
				if (resource === "store" && operation === "getCountries") {
					const variables: IDataObject = {};
					const countries_id = this.getNodeParameter("getCountries_countries_id", i) as number;
					variables.countries_id = countries_id;
					const status = this.getNodeParameter("getCountries_status", i) as number;
					variables.status = status;
					const limit = this.getNodeParameter("getCountries_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getCountries_offset", i) as number;
					variables.offset = offset;
  ...
  get_countries(countries_id: $countries_id, status: $status, limit: $limit, offset: $offset) {
    ...
    totalCountries
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.get_countries) returnData.push(responseData.data.get_countries);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
    ...
    modifyOrderProduct (orderid: $orderid, input: $input) {
        ...
        id
				...
				if (resource === "basket" && operation === "setUserBasket") {
					...
					const input = JSON.parse(this.getNodeParameter("setUserBasket_input", i) as string);
					variables.input = input;
    ...
    setUserBasket (userId: $userId, action: $action, itemIndex: $itemIndex, input: $input) {
        ...
        basket_id
				...
				if (resource === "basket" && operation === "getUserBasket") {
					const variables: IDataObject = {};
					const user_id = this.getNodeParameter("getUserBasket_user_id", i) as number;
  ...
  getUserBasket( user_id: $user_id ) {
    ...
    totalBaskets
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.getUserBasket) returnData.push(responseData.data.getUserBasket);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "product" && operation === "setProductsImageGallery") {
					...
					const optimizeimg = this.getNodeParameter("setProductsImageGallery_optimizeimg", i) as number;
					variables.optimizeimg = optimizeimg;
					const input = JSON.parse(this.getNodeParameter("setProductsImageGallery_input", i) as string);
					variables.input = input;
  ...
  setProductsImageGallery(products_id: $products_id, optimizeimg: $optimizeimg, input: $input) {
    ...
    message {
      ...
      message
				...
				if (resource === "product" && operation === "productsImageGallery") {
					const variables: IDataObject = {};
					const products_image_gallery_id = this.getNodeParameter("productsImageGallery_products_image_gallery_id", i) as number;
					variables.products_image_gallery_id = products_image_gallery_id;
					const products_id = this.getNodeParameter("productsImageGallery_products_id", i) as number;
					variables.products_id = products_id;
					const corporate_id = this.getNodeParameter("productsImageGallery_corporate_id", i) as number;
					variables.corporate_id = corporate_id;
					const limit = this.getNodeParameter("productsImageGallery_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("productsImageGallery_offset", i) as number;
					variables.offset = offset;
    ...
    products_image_gallery ( products_image_gallery_id: $products_image_gallery_id, products_id: $products_id,corporate_id: $corporate_id, limit: $limit, offset: $offset) {
        ...
        totalProductsImageGallery
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.productsImageGallery) returnData.push(responseData.data.productsImageGallery);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
  ...
  accountSummary(storeid: $storeid, limit: $limit, offset: $offset) {
    ...
    remainingPaidLimit
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.accountSummary) returnData.push(responseData.data.accountSummary);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
  ...
  storeCreditSummary(storeid: $storeid, user_id: $user_id, limit: $limit, offset: $offset) {
    ...
    remainingCredit
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.storeCreditSummary) returnData.push(responseData.data.storeCreditSummary);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "order" && operation === "getPaymentTermMaster") {
					const variables: IDataObject = {};
					const term_id = this.getNodeParameter("getPaymentTermMaster_term_id", i) as number;
					variables.term_id = term_id;
					const limit = this.getNodeParameter("getPaymentTermMaster_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("getPaymentTermMaster_offset", i) as number;
					variables.offset = offset;
  ...
  get_payment_term_master(term_id: $term_id, limit: $limit, offset: $offset) {
    ...
    totalPaymentTermMaster
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.get_payment_term_master) returnData.push(responseData.data.get_payment_term_master);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
    ...
    setQuantityBasedAttributePrice(inputs: $inputs) {
        ...
        id
				...
				if (resource === "additionalOption" && operation === "productAdditionalOptions") {
					const variables: IDataObject = {};
					const products_id = this.getNodeParameter("productAdditionalOptions_products_id", i) as number;
					variables.products_id = products_id;
					const limit = this.getNodeParameter("productAdditionalOptions_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("productAdditionalOptions_offset", i) as number;
					variables.offset = offset;
    ...
    product_additional_options (products_id: $products_id, limit: $limit, offset: $offset) {
        ...
        totalProductAdditionalOptions
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.product_additional_options) returnData.push(responseData.data.product_additional_options);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "additionalOption" && operation === "productsAttributePrice") {
					const variables: IDataObject = {};
					const attribute_id = this.getNodeParameter("productsAttributePrice_attribute_id", i) as number;
					variables.attribute_id = attribute_id;
					const size_id = this.getNodeParameter("productsAttributePrice_size_id", i) as number;
					variables.size_id = size_id;
					const limit = this.getNodeParameter("productsAttributePrice_limit", i) as number;
					variables.limit = limit;
					const offset = this.getNodeParameter("productsAttributePrice_offset", i) as number;
					variables.offset = offset;
    ...
    products_attribute_price(attribute_id: $attribute_id, size_id: $size_id, limit: $limit, offset: $offset) {
        ...
        totalProductsAttributePrice
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.products_attribute_price) returnData.push(responseData.data.products_attribute_price);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				if (resource === "shipToMultipleAddress" && operation === "shipToMultipleAddress") {
					const variables: IDataObject = {};
					const order_id = this.getNodeParameter("shipToMultipleAddress_order_id", i) as number;
					if (!Number.isInteger(Number(order_id)) || Number(order_id) <= 0) {
						throw new NodeOperationError(this.getNode(), 'Set a positive Order ID to retrieve ship-to-multiple addresses', { itemIndex: i });
					}
					variables.order_id = order_id;
    ...
    shipToMultipleAddress (order_id: $order_id) {
        shipToMultipleAddress {
			...
			ship_to_multiple_address_extra_field
					...
					const responseData = await requestGraphql({ query: query, variables });
					if (responseData && responseData.data && responseData.data.shipToMultipleAddress) returnData.push(responseData.data.shipToMultipleAddress);
					else if (responseData && responseData.errors) throw new NodeOperationError(this.getNode(), `GraphQL Error: ${JSON.stringify(responseData.errors)}`, { itemIndex: i });
				}
				...
				if (resource === 'customer' && operation === 'update') {
					...
					if (updateFields.external_ref !== undefined) input.external_ref = updateFields.external_ref;
							...
							try {
								...
								if (responseData && responseData.data && responseData.data.customers) {
									...
									if (Array.isArray(customers) && customers.length > 0) {
										...
										if (hasMorePages) {
											await sleep(Math.round(adaptiveDelay));
										}
									} else {
										hasMorePages = false;
									}
									...
									throw new NodeApiError(this.getNode(), toNodeApiErrorResponse(error), { itemIndex: i });
								}
						}
			...
			if (resource === 'product' && operation === 'getManyDetailed') {
				...
				const buildManyDetailedVariables = (limit?: number, offset?: number): IDataObject => {
					const variables: IDataObject = {};
					if (limit !== undefined) variables.limit = limit;
					else if (queryParameters.limit) variables.limit = queryParameters.limit;
					if (offset !== undefined) variables.offset = offset;
					else if (queryParameters.offset) variables.offset = queryParameters.offset;
					if (queryParameters.status !== undefined) variables.status = queryParameters.status;
					if (queryParameters.all_store !== undefined) variables.all_store = queryParameters.all_store;
					if (queryParameters.externalCatalogue !== undefined) variables.externalCatalogue = queryParameters.externalCatalogue;
					return variables;
				...
				const pushManyDetailedProducts = (responseData: IDataObject): number => {
					if (responseData && responseData.data && (responseData.data as IDataObject).products_details) {
						const response = ((responseData.data as IDataObject).products_details || {}) as IDataObject;
						const products = response.products;
						const totalProducts = response.totalProducts;
						const currentCount = response.currentCount;
						if (Array.isArray(products)) {
							for (const product of products as IDataObject[]) {
								returnData.push({
									...product,
									_totalProducts: totalProducts,
				...
				if (fetchAllPages) {
					...
					while (hasMorePages && pageCount < maxPages) {
						...
						if (hasMorePages) await sleep(pageDelay);
					}
					} else {
						const responseData = await requestGraphql({
							query: query.trim(),
		...
		if (resource === 'status' && operation === 'getStatus') {
				...
				throw new NodeOperationError(this.getNode(), getErrorMessage(error), { itemIndex: i });
			}
		}
