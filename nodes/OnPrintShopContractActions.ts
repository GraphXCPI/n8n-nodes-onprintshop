...
type DomainName = 'onPrintShopProducts' | 'onPrintShopProductBuilder' | 'onPrintShopCustomers' | 'onPrintShopStoreAdmin';

const intField = (displayName: string, name: string, resources: string[], operations: string[], required = false): INodeProperties => ({
	displayName,
	name,
	type: 'number',
	default: 0,
...
const stringField = (displayName: string, name: string, resources: string[], operations: string[], required = false, rows = 1): INodeProperties => ({
	displayName,
	name,
	type: 'string',
	default: '',
...
const NEW_RESOURCES: Partial<Record<DomainName, Array<{ name: string; value: string }>>> = {
...
const NEW_OPERATIONS: Record<string, Array<{ name: string; value: string; action: string }>> = {
...
const queryProperties: INodeProperties[] = [
...
const adminExtraFieldValues: INodeProperties = {
...
const batchInputMode = (resource: string): INodeProperties => ({
...
const mutationProperties: INodeProperties[] = [
...
function operationProperty(resource: string, options: Array<{ name: string; value: string; action: string }>): INodeProperties {
	...
	if (resource === 'basket') property.default = 'get';
	if (resource === 'notification') property.default = 'send';
	return property;
}

function operationAppliesTo(property: INodeProperties, resource: string): boolean {
	const show = property.displayOptions?.show as Record<string, unknown> | undefined;
	return Array.isArray(show?.resource) && show.resource.includes(resource);
}

export function extendOnPrintShopDomainDescription(description: INodeTypeDescription, domainName: string): INodeTypeDescription {
	...
	for (const resource of resources) description.properties.push(operationProperty(resource.value, NEW_OPERATIONS[resource.value]));
	...
	if (domain === 'onPrintShopCustomers') {
		...
		if (addressOperation && Array.isArray(addressOperation.options)) addressOperation.options.push({ name: 'Create, Update, or Delete', value: 'set', action: 'Create update or delete a customer address' });
	}
	if (domain === 'onPrintShopStoreAdmin') {
		...
		if (faqOperation && Array.isArray(faqOperation.options)) faqOperation.options.push({ name: 'Create, Update, or Delete', value: 'set', action: 'Create update or delete an faq' });
	}
...
function optionalInt(context: IExecuteFunctions, name: string, itemIndex: number): number | undefined {
	const value = context.getNodeParameter(name, itemIndex, 0) as number;
	return value || undefined;
}

function pageVariables(context: IExecuteFunctions, resource: string, itemIndex: number): IDataObject {
	return { limit: context.getNodeParameter(`${resource}_limit`, itemIndex) as number, offset: context.getNodeParameter(`${resource}_offset`, itemIndex) as number };
}

function parseObjectArray(value: unknown, label: string, context: IExecuteFunctions, itemIndex: number): IDataObject[] {
	let parsed = value;
	if (typeof parsed === 'string') {
		try { parsed = JSON.parse(parsed); } catch { throw new NodeOperationError(context.getNode(), `${label} must be valid JSON`, { itemIndex }); }
	}
	if (!Array.isArray(parsed) || parsed.some((entry) => !entry || typeof entry !== 'object' || Array.isArray(entry))) {
		throw new NodeOperationError(context.getNode(), `${label} must be a JSON array of objects`, { itemIndex });
	}
	return parsed as IDataObject[];
}

function normalizeNestedRows(value: IDataObject): IDataObject {
	const normalized = compactObject({ ...value });
	if (value.attributes) normalized.attributes = rowsFromFixedCollection(value.attributes, 'attribute').map((row) => compactObject(row));
	if (value.admin_extra_fields) normalized.admin_extra_fields = rowsFromFixedCollection(value.admin_extra_fields, 'field').map((row) => compactObject(row));
	return normalized;
}

export async function executeOnPrintShopContractAction(context: IExecuteFunctions): Promise<INodeExecutionData[][] | null> {
	...
	if (!supported.has(`${resource}.${operation}`)) return null;

	const request = await createOnPrintShopGraphqlClient(context);
	...
	for (let itemIndex = 0; itemIndex < context.getInputData().length; itemIndex++) {
		try {
			...
			if (resource === 'adminExtraField' && operation === 'getMany') {
				const variables = compactObject({ admin_extra_field_id: optionalInt(context, 'adminExtraField_id', itemIndex), available_to: context.getNodeParameter('adminExtraField_availableTo', itemIndex) as string, status: context.getNodeParameter('adminExtraField_status', itemIndex) as string, ...pageVariables(context, resource, itemIndex) });
				...
				const mode = context.getNodeParameter('adminExtraField_inputMode', itemIndex, 'form') as string;
				const inputs = mode === 'json' ? parseObjectArray(context.getNodeParameter('adminExtraField_inputsJson', itemIndex), 'Admin Extra Fields JSON', context, itemIndex) : rowsFromFixedCollection(context.getNodeParameter('adminExtraField_inputs', itemIndex), 'item').map(normalizeNestedRows);
				...
				const variables = compactObject({ store_location_id: optionalInt(context, 'storeLocation_id', itemIndex), location_id_code: context.getNodeParameter('storeLocation_code', itemIndex) as string, status: context.getNodeParameter('storeLocation_status', itemIndex) as string, ...pageVariables(context, resource, itemIndex) });
				...
				const mode = context.getNodeParameter('storeLocation_inputMode', itemIndex, 'form') as string;
				const inputs = mode === 'json' ? parseObjectArray(context.getNodeParameter('storeLocation_inputsJson', itemIndex), 'Store Locations JSON', context, itemIndex) : rowsFromFixedCollection(context.getNodeParameter('storeLocation_inputs', itemIndex), 'item').map(normalizeNestedRows);
				...
				const variables = compactObject({ products_id: optionalInt(context, 'productAdditionalOption_productsId', itemIndex), ...pageVariables(context, resource, itemIndex) });
				...
				const variables = compactObject({ attribute_id: optionalInt(context, `${prefix}_attributeId`, itemIndex), size_id: optionalInt(context, `${prefix}_sizeId`, itemIndex), ...pageVariables(context, resource, itemIndex) });
				...
				data = await request('query getUserBasket($user_id: Int!) { getUserBasket(user_id: $user_id) { baskets { basket_id user_id cart_detail cart_count date } totalBaskets } }', { user_id: context.getNodeParameter('basket_userId', itemIndex) as number }, itemIndex);
				result = data.getUserBasket;
			} else if (resource === 'basket') {
				const action = context.getNodeParameter('basket_action', itemIndex) as string;
				const items = rowsFromFixedCollection(context.getNodeParameter('basket_items', itemIndex, {}), 'item').map((item) => {
					const options = rowsFromFixedCollection(item.options, 'option');
					return compactObject({ ...item, options: options.length ? options.map((entry) => ({ [String(entry.key)]: entry.value })) : undefined });
				});
				data = await request('mutation setUserBasket($user_id: Int!, $action: String!, $item_index: Int!, $input: SetUserBasketInput!) { setUserBasket(user_id: $user_id, action: $action, item_index: $item_index, input: $input) { result message id } }', { user_id: context.getNodeParameter('basket_userId', itemIndex) as number, action, item_index: context.getNodeParameter('basket_itemIndex', itemIndex) as number, input: { items } }, itemIndex);
				result = data.setUserBasket;
			} else if (resource === 'notification') {
				const input = compactObject({ cc: context.getNodeParameter('notification_cc', itemIndex) as string, bcc: context.getNodeParameter('notification_bcc', itemIndex) as string, subject: context.getNodeParameter('notification_subject', itemIndex) as string, body: context.getNodeParameter('notification_body', itemIndex) as string, attachments: context.getNodeParameter('notification_attachments', itemIndex) as string, msg_type: context.getNodeParameter('notification_messageType', itemIndex) as string });
				data = await request('mutation notifyUser($usertype: UserNotifyTypeEnum!, $cust_id: Int, $input: UserNotifyInput!) { notifyUser(cust_id: $cust_id, usertype: $usertype, input: $input) { result message id } }', { usertype: context.getNodeParameter('notification_userType', itemIndex) as string, cust_id: optionalInt(context, 'notification_customerId', itemIndex), input }, itemIndex);
				result = data.notifyUser;
			} else if (resource === 'customerAddress') {
				const input = compactObject({ user_id: optionalInt(context, 'customerAddress_userId', itemIndex), address_book_id: optionalInt(context, 'customerAddress_addressBookId', itemIndex), firstname: context.getNodeParameter('customerAddress_firstName', itemIndex) as string, lastname: context.getNodeParameter('customerAddress_lastName', itemIndex) as string, companyname: context.getNodeParameter('customerAddress_companyName', itemIndex) as string, street_address: context.getNodeParameter('customerAddress_streetAddress', itemIndex) as string, suburb: context.getNodeParameter('customerAddress_suburb', itemIndex) as string, postcode: context.getNodeParameter('customerAddress_postcode', itemIndex) as string, city: context.getNodeParameter('customerAddress_city', itemIndex) as string, state: context.getNodeParameter('customerAddress_state', itemIndex) as string, country: context.getNodeParameter('customerAddress_country', itemIndex) as string, phone_number: context.getNodeParameter('customerAddress_phoneNumber', itemIndex) as string, delete: context.getNodeParameter('customerAddress_delete', itemIndex) ? 1 : undefined });
				...
				const variables = compactObject({ storeid: optionalInt(context, 'accountSummary_storeId', itemIndex), ...pageVariables(context, resource, itemIndex) });
				...
				const variables = compactObject({ storeid: optionalInt(context, 'storeCredit_storeId', itemIndex), user_id: optionalInt(context, 'storeCredit_userId', itemIndex), ...pageVariables(context, resource, itemIndex) });
				...
				const input = compactObject({ faq_id: optionalInt(context, 'faq_faqId', itemIndex), faqcat_id: context.getNodeParameter('faq_categoryId', itemIndex) as number, corporate_id: optionalInt(context, 'faq_corporateId', itemIndex), status: context.getNodeParameter('faq_status', itemIndex) as string, sort_order: context.getNodeParameter('faq_sortOrder', itemIndex) as number, faq_type: context.getNodeParameter('faq_type', itemIndex) as string, product_category_ids: context.getNodeParameter('faq_productCategoryIds', itemIndex) as string, faq_question: context.getNodeParameter('faq_question', itemIndex) as string, faq_answer: context.getNodeParameter('faq_answer', itemIndex) as string });
				data = await request('mutation setFaq($input: FaqInput!) { setFaq(input: $input) { result message id } }', { input }, itemIndex);
				result = data.setFaq;
			}
			if (result === undefined) throw new NodeOperationError(context.getNode(), `OnPrintShop response did not include ${resource}`, { itemIndex });
			output.push(...resultItems(result).map((entry) => ({ ...entry, pairedItem: { item: itemIndex } })));
		} catch (error) {
			if (context.continueOnFail()) output.push({ json: { error: (error as Error).message }, pairedItem: { item: itemIndex } });
			else throw new NodeOperationError(context.getNode(), error as Error, { itemIndex });
		}
	}
	return [output];
}
